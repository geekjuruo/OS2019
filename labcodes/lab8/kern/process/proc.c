...
alloc_proc(void) {
     ...
     */
     //LAB5 2016011372 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized	
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2016011372 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2016011372 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2016011372
    //LAB8:EXERCISE2 2016011372  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    */
	...
	proc->parent = current;//set child proc's parent to current process
	assert(proc->wait_state == 0);//make sure current process's wait_state is 0
    //    2. call setup_kstack to allocate a kernel stack for child process
	int flagKstack = setup_kstack(proc);
	if(flagKstack == -4) {
		goto bad_fork_cleanup_proc;	
	}
    // for lab8 copy files from parent to children process
    int flagFiles = copy_files(clone_flags, proc);
    if(flagFiles != 0) {
	...
	if(flagMm != 0) {
		goto bad_fork_cleanup_fs;	 // for lab8 error	
	}
    //    4. call copy_thread to setup tf & context in proc_struct
	...
	local_intr_save(intr_flag);//保证pid的唯一性设置的中断屏蔽
	{
		...
		set_links(proc); //insert proc_struct into hash_list && proc_list, set the relation links of process
	}
	local_intr_restore(intr_flag);
    //    6. call wakeup_proc to make the new child process RUNNABLE
	wakeup_proc(proc);
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2016011372  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
